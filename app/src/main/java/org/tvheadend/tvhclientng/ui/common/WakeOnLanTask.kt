...
import org.tvheadend.tvhclientng.util.extensions.executeAsyncTask
...
class WakeOnLanTask(lifecycleScope: LifecycleCoroutineScope, context: Context, private val connection: Connection) {
    ...
    private var sendWakeOnLanPacket = SendWakeOnLanPacket(connection)
    ...
    init {
        lifecycleScope.executeAsyncTask(onPreExecute = {
            ...
            val status: Int = sendWakeOnLanPacket.prepareAndSend()
            ...
            when (it) {
                ...
                WOL_INVALID_MAC -> {
                    ...
                    message = context.getString(R.string.wol_error, "${connection.wolMacAddress}:${connection.port}", sendWakeOnLanPacket.exception?.localizedMessage)
                }
            }
    ...
    companion object {
        ...
        const val WOL_SEND = 0
        const val WOL_SEND_BROADCAST = 1
        const val WOL_INVALID_MAC = 2
        const val WOL_ERROR = 3
    }
}

class SendWakeOnLanPacket(val connection: Connection) {

    var exception: Exception? = null

    fun prepareAndSend(): Int {
        ...
        if (!validateMacAddress(connection.wolMacAddress)) {
            return WakeOnLanTask.WOL_INVALID_MAC
        } else {
            ...
            try {
                val uri = URI(connection.serverUrl)
                ...
                return if (!connection.isWolUseBroadcast) {
                    WakeOnLanTask.WOL_SEND
                } else {
                    WakeOnLanTask.WOL_SEND_BROADCAST
                }
            } catch (e: Exception) {
                this.exception = e
                return WakeOnLanTask.WOL_ERROR
            }
        }
    }
